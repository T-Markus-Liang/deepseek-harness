...
A notice injected during teardown is not read by a model when that parent is disposed next, which every teardown caller does: the disposal cancel clears the unclaimed message and the log keeps the insert/cancel pair as the record. Making teardown delivery readable after resume requires either the offline mailbox above or a change to disposal of durable pending work. Disposal discards every unclaimed inbox item, including user input, so changing that behavior is a core-agent decision rather than a settlement-delivery detail. After resume, the parent does not receive the missed notice automatically, but an exact live direct parent can call `get_agent_result` to read the child Session's latest recorded output and stop reason without resuming the child or starting another model turn. `list_agents` remains discovery only; `SubagentListEntry.activity` does not encode a result.
