...
| `ctx.workspaceInspector` | `core` | [`workspace-inspector`](../packages/workspace/workspace-inspector) | - | `apiproxy` | - | Answers the browser workbench's file-tree, preview, and Git reads constrained to one registered workspace root; no write or shell path exists. |
...
| `ctx.verifier` | `seam` | [`verifier`](../packages/verifier/verifier) | [`verifier-python`](../packages/verifier/verifier-python) | [`tool-verify-candidates`](../packages/verifier/tool-verify-candidates), [`tool-best-of-n`](../packages/verifier/tool-best-of-n) | - | Providers rank complete candidate trajectories through explicit selection requests; opt-in consumers either read authorized durable Sessions or generate isolated subagent candidates and promote one patch. |
