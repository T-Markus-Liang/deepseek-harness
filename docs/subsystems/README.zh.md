...
| [verifier.md](verifier.md) | 完整轨迹 selection 请求、标准、候选坐标排名、token 用量与 `ctx.verifier` 服务 |
...
| [session-stats.md](session-stats.md) | `sessionStats` 投影单元：全日志轮/步计数与墙钟时间、实时 `openStep` 与进行中 `pendingCalls`，以及可选的 `stallThresholdMs` 卡死监控 |
