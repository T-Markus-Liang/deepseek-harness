...
读取你的一个直接可继续后台 subagent 的最新记录结果，不恢复它、不发送消息，也不启动模型轮次。可在可能错过完成通知时使用。active 表示 agent 仍有在线 Activation；inactive 表示它处于轮次之间且只存在于存储中，以后仍可恢复。
...
根据 agent id 请求取消后台 agent 的当前轮次。目标可以是你的直接子级，也可以是在你下方创建的更深层 agent。只有当前轮次会停止：已经排队发给该 agent 的消息会一直搁置到后续的 send_message；它启动的 agent 会继续运行；该 agent 本身仍可接受后续操作。停止请求被接受后，此调用立即返回，因此目标可能还会短暂运行。结果会区分已请求中断、目标正在结算以及不存在在线的继续执行 Activation。
...
这些是控制可继续后台 subagent 的全局命名工具：绑定提供方的 `tool-subagent` 实例注册不同的委派工具；本包注册一次 `send_message`、`get_agent_result` 和 `interrupt_agent`，另由 `list_agents` 通过单独加载的 `/list-agents` 插件提供，其目录行使用 sessionProjections 和实时 Agent 注册表。
