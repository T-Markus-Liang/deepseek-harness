...
describe('workspace browser rows', () => {
  ...
  it('blank New Session rows carry no menu, no time label, and no hover-card time', () => {
    ...
    try {
      ...
      // Right-click carries no verbs either: the blank row stays inert.
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, archive, and delete', () => {
    ...
    expect(onRename).toHaveBeenCalledOnce()
    expect(onOpen).not.toHaveBeenCalled()
    // Delete is destructive: danger styling, and the dispatch carries the title
    // so the confirmation dialog can name the target.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
