...
- **模态声明不经验证，且多声明的后果超出本轮**：没有任何环节会去询问端点接受什么，因此声明了网关并不提供的 `image` 的模型不会在这里被拦下，而是由提供方在轮次中途拒绝。prompt 准入在构造请求之前就把用户消息持久化提交，于是被拒绝的图片留在会话日志里，该模型会不断重发它。即使会话保留图片，模型选择仍然可用；但把这些图片发送给纯文本模型时，prompt 准入会给出明确错误。此时应切换到支持图片的模型、在图片之前 fork，或开启新会话。发送失败时把尚未消费的图片消息从日志中回滚出去这件事已暂缓。
