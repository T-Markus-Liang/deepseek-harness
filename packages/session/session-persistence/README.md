...
Detached-session deletion and relocation are exposed separately as `ctx.sessionLifecycle` by first-party JSONL and SQLite providers. The optional `SessionLifecycle` service keeps the rc.8 `SessionPersistence` read/write API stable for future upstream upgrades. Its `remove(id)` and `move(id, newCwd)` methods reject live or in-memory sessions through the shared coordinator and treat absent ids as idempotent no-ops.
...
- **No general deletion or retention API** — pruning stored sessions is an out-of-band backend operation; first-party providers expose deletion and relocation through optional `ctx.sessionLifecycle`.
