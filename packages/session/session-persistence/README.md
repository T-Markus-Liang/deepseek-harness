...
| `remove(id): Promise<void>` | Remove every durable artifact for a detached session. The coordinator rejects live or retiring sessions and invalidates its detached state. Missing ids are idempotent. |
| `move(id, newCwd): Promise<void>` | Move a detached session to a new project directory and rewrite its persisted cwd. The coordinator serializes the operation with persistence writes; missing ids are idempotent. |
