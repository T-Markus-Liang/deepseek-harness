...
spawn 声明 `{ outputSchema: true, depthLimit: true, toolFilter: true, persona: true, workspaceCwd: true }`，因为它控制子 agent 的创建窗口，能够强制执行全部五项功能。`workspaceCwd` 会替换子 Session header 中继承自父级的 cwd。
